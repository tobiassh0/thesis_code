...
def v_MCI(sim_lst, xi2, plot=False):
	v_ph = np.zeros(len(sim_lst)) ; v_gr = np.zeros(len(sim_lst))
	w_MCI = np.zeros(len(sim_lst)) ; k_MCI = np.zeros(len(sim_lst))
	vA = np.zeros(len(sim_lst))
	kmax = 100 ; wmax = 50
	i=0
	home = os.getcwd()
	for sim in sim_lst:
		...
		_k_MCI=karr[MCI[1]+oset] # unitless
		_w_MCI=warr[MCI[0]+oset] # unitless
		w_MCI[i] = _w_MCI
		k_MCI[i] = _k_MCI
	#	plt.imshow(np.log10(FT2d),**kwargs,extent=[0,kmax,0,wmax])
	#	plt.scatter(_k_MCI,_w_MCI,marker='s')
		...
		k2 = k2 * _vA/wcyc
		omegas = omegas/wcyc
		# calculate v_phase (_w_MCI / _k_MCI)
		v_ph[i] = _w_MCI/_k_MCI * _vA # units of m/s
		# calculate gradient (v_group = [dw/dk]|_MCI)
		dw = (omegas[-1])/len(omegas)
		kgrad = np.gradient(k2,dw)
		kpos = int((np.abs(k2[1:] - _k_MCI)).argmin())+1
		print(k2[kpos],_k_MCI,_w_MCI)
		v_gr[i] = _vA/kgrad[kpos] # units of m/s
		# correcting _vA
	#	v_gr[i] = v_gr[i]*_vA
	#	v_ph[i] = v_ph[i]*_vA
		os.chdir(home)
	...
	# returns the speeds and frequencies & wavenumbers both normalised (to vA and wcyc)
	return v_ph/vA, v_gr/vA, w_MCI, k_MCI 
