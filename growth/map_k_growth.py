...
## deuteron harmonics
for a in np.arange(0,axs.shape[0],1):
...
#		ax[t].scatter(omegas/wnorm,growthRatesMean/wnorm,marker='o',facecolors=colors[SimIndex])
	...
	ax[t].set_ylabel(r'$\gamma/\Omega_D$',**tnrfont)
	t+=1

# time range annotations
ax[0].text(1.,3.,r'$0<t/\tau_{cD}<0.5$',color='black',fontsize=18,bbox=dict(facecolor='white',edgecolor='black',boxstyle='square,pad=0.5'))
ax[1].text(1.,3.,r'$0.5<t/\tau_{cD}<2.0$',color='black',fontsize=18,bbox=dict(facecolor='white',edgecolor='black',boxstyle='square,pad=0.5'))
# x-label
