...
## spectrograms
try:
	Sarr, freqs, time = read_pkl('Sxftarray_nfft_3291_noverlap_1645')
	Spower = read_pkl('Sxpower_nfft_1646_noverlap_823')
except:
	fieldmatrix = load_batch_fieldmatrix([],quantity)
	Spower, Sarr, freqs, time = getSpectrogram(fieldmatrix,times,majspec='Protons',nfo=[5,2],minspec='FElectrons',plot=False)
print(Sarr.shape)
#fig=plt.figure(figsize=(10,10))
#ax=fig.add_subplot(111,projection='3d')
#img = ax.scatter(Sarr[:,0],Sarr[:,2],Sarr[:,1],marker='s',color='green')
#ax.set_xlabel('pos [x]',**tnrfont)
#ax.set_zlabel('freqs [Hz]',**tnrfont)
#ax.set_ylabel('time [s]',**tnrfont)
#plt.show()
nx,nf,nt = Sarr.shape
for x in range(0,nx,1000):
	plt.imshow(Sarr[x,:,:],extent=[0,time[-1],freqs[0],freqs[-1]/fcp],aspect='auto',interpolation='nearest')
	plt.show()
sys.exit()
